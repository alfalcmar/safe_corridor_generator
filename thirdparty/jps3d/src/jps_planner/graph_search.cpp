...
bool GraphSearch::plan(int xStart, int yStart, int xGoal, int yGoal, bool useJps, int maxExpand, bool plan_to_nearest) {
  use_2d_ = true;
  pq_.clear();
  path_.clear();
  hm_.resize(xDim_ * yDim_);
...
bool GraphSearch::plan(int xStart, int yStart, int zStart, int xGoal, int yGoal, int zGoal, bool useJps, int maxExpand, bool plan_to_nearest) {
  use_2d_ = false;
  pq_.clear();
  path_.clear();
  hm_.resize(xDim_ * yDim_ * zDim_);
  ...
  return plan(currNode_ptr, maxExpand, start_id, goal_id, plan_to_nearest);
}

bool GraphSearch::plan(StatePtr& currNode_ptr, int maxExpand, int start_id, int goal_id, bool plan_to_nearest) {
  // Insert start node
  StatePtr nearestNode_ptr = std::make_shared<State>(State(0, 0, 0, 0, 0, 0, 0));
  double   nearest_h_val   = std::numeric_limits<double>::infinity();
  ...
  while (true) {
    ...
    for (int s = 0; s < (int)succ_ids.size(); s++) {
      ...
      if (tentative_gval < child_ptr->g) {
        ...
        if (child_ptr->h < nearest_h_val) {
          nearestNode_ptr = child_ptr;
          nearest_h_val   = child_ptr->h;
        }
    ...
    if (maxExpand > 0 && expand_iteration >= maxExpand) {
      ...
      if (plan_to_nearest) {
        currNode_ptr = nearestNode_ptr;
        printf("Nearest node to goal = [%d, %d, %d] \n", currNode_ptr->x, currNode_ptr->y, currNode_ptr->z);
      } else {
        return false;
      }
    }
...
bool GraphSearch::jump(int x, int y, int z, int dx, int dy, int dz, int& new_x, int& new_y, int& new_z) {
  ...
  for (int k = 0; k < num_neib - 1; ++k) {
    ...
    if (jump(new_x, new_y, new_z, jn3d_->ns[id][0][k], jn3d_->ns[id][1][k], jn3d_->ns[id][2][k], new_new_x, new_new_y, new_new_z))
      return true;
  }
  ...
  for (int dz = -1; dz <= 1; ++dz) {
    for (int dy = -1; dy <= 1; ++dy) {
      for (int dx = -1; dx <= 1; ++dx) {
        ...
        for (int dev = 0; dev < nsz[norm1][1]; ++dev) {
          FNeib(dx, dy, dz, norm1, dev, f1[id][0][dev], f1[id][1][dev], f1[id][2][dev], f2[id][0][dev], f2[id][1][dev], f2[id][2][dev]);
        }
        id++;
      }
    }
